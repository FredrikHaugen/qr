...
function ClicksCard({ days }) {
  const [clicksData, setClicksData] = useState([]);
  const [isExpanded, setIsExpanded] = useState(false);

  const generateClicksData = (days) => {
    const data = [];
    for(let i=0; i<days; i++){
      const date = new Date();
      date.setDate(date.getDate() - i);
      data.push({ date, clicks: Math.floor(Math.random() * 100) });
    }
    return data;
  };
  ...
  const displayedData = isExpanded ? clicksData : clicksData.slice(0, 5);
        ...
        {displayedData.map((data, index) => (
          <div key={index} className='flex justify-center items-center space-x-2'>
            <p className='text-slate-800 text-xl font-thin'>
              {displayDate(data.date)} - Clicks:</p>
            <span className='text-slate-800 text-2xl font-thin'> {data.clicks}
            </span>
          </div>
        ))}
      </div>
